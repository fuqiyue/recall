...
**需求拆解（2026-08-16 按 RULE-014 需求保全补录；原 CHG-20260816-001 与本记录
在同一提交中立案并归档，正文未单独进入 git 历史——此事实即为需求保全规则的立法依据）**：
- raw_request: "请你结合目前给的 Recall 可能的逻辑问题（按严重度排），综合继续排查问题，然后告诉我总体如何修改" + "认可你的问题，请帮我逐个修改"（2026-08-16）
- decomposition: 修复 recall new 占位符断链；自动保存文件清单与私人文件保护；回填双通道；INT 编号统一并补登记；validate 三处对账/撞号/意图层/CHG 三字段/占位符检查；漂移哨兵；CLAUDE.md 平行真源收敛；conflicts 局限声明；历史断链回填（VER-20260808-001）
- fit_analysis: 强化 INT-20260816-005（sync/回填）与 INT-20260816-008（validate）既有意图，补登记 INT-20260816-001；不触碰 UXI-001..004
