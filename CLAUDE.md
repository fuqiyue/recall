...
1. 先读根 `logic_readme.md`（宪法：全局规则、领域目录、功能意图/用户流程层），再读根 `logic_change.md`（修宪议案 + 全项目活跃议案索引），然后 `recall route <目标路径或关键词>` 只读命中领域的 `logic_domains/<domain>/logic_readme.md` + `logic_change.md`（部门法与其议案），再读相关代码、测试和必要的运行证据；`logic_version/` 历史仅在现行文档引用或需要追溯时按 ID 读取
...
4. 代码改动后更新并运行相关测试，报告结果与 `docs_impact`；现行规则、锚点、契约或验证入口实际变化时在同一变更中更新所属 `logic_readme.md`（领域规则改领域文档，全局规则/意图层/登记表改宪法）
