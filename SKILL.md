...
5. 新请求与现行规则、活跃议案或已确认意图存在实质矛盾，或模糊点会改变范围/语义/兼容/数据安全/方案选择时，先列明新旧来源、具体矛盾、可行选项、主要影响和建议，再向当前用户或授权决策方请求明确选择；确认前不得实施受影响部分
...
1. 先读根 `logic_readme.md` 的文档控制、范围登记和代码地图索引，再按 `scope_path` 与稳定锚点读取相关规则、契约与不变量
